    ...
    <section id="faq" className="px-6 py-32 bg-[#FAF9F6]">
      ...
      <div className="max-w-4xl mx-auto space-y-16">
        <div className="space-y-6">
          <div className="flex items-center gap-4"><div className="h-px w-12 bg-[#C5A065]/40"/><span className="text-[11px] font-bold uppercase tracking-[0.3em] text-[#A67A3D]">FAQ</span></div>
        ...
        <div className="space-y-4">
          {generalFaqs.map((f, i) => (
            <div key={i} className={`rounded-[2rem] border transition-all ${openIndex === i ? 'bg-white border-[#C5A065]/30 shadow-xl' : 'border-[#E8DFD1]/60'}`}>
              ...
              {openIndex === i && <div className="px-8 pb-8 text-sm text-[#423C35] leading-relaxed">{f.answer}</div>}
            </div>
          ))}
        </div>
      </div>
